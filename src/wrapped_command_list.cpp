...
HRESULT WrappedCommandList::QueryInterface(REFIID riid, void** ppvObj)
{
    ...
    // Return 'this' for every CL version we might be asked for, after
    // verifying the real object supports it.
    ...
    {
        *ppvObj = static_cast<ID3D12GraphicsCommandList*>(this);
        AddRef();
        return S_OK;
    }

    // For CL1+: the real object supports them; return the real pointer.
    // Our intercepted methods (SetGraphicsRootSignature, SetComputeRootSignature,
    // DrawInstanced, DrawIndexedInstanced, Dispatch) are all on CL0, so bypassing
    // our wrapper for CL1+ calls is safe and correct.
...
ULONG WrappedCommandList::Release()
{
    ULONG c = --m_refCount;
    if (!c) delete this;
    return c;
...
void WrappedCommandList::BindStereoParam(bool isGraphics)
{
    ...
    if (!m_pDevice->GetRSMeta(pRS, meta)) return;
    ...
    LOG_TRACE("BindStereoParam: slot=%u isGraphics=%d", meta.stereoParamIdx, (int)isGraphics);
}

// ===========================================================================
// KEY OVERRIDES: RS binding + draw/dispatch (stereo auto-bind)
// ===========================================================================
...
void WrappedCommandList::DrawInstanced(UINT vc, UINT ic, UINT sv, UINT si)
{
    if (m_graphicsDirty) { BindStereoParam(true); m_graphicsDirty = false; }
    m_pReal->DrawInstanced(vc, ic, sv, si);
}
